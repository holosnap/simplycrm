...
A lightweight, shared contact-management CRM for a small team (2–20 users). No sales pipeline/deals in v1 — this is a team rolodex with notes, activity history, tasks, and light organization (tags/custom fields), plus admin/user roles.

## 1. Scope

**In scope (v1):**
- Contacts and the companies/organizations they belong to
- Notes & activity timeline per contact (calls, emails, meetings, freeform notes)
...
| Database | PostgreSQL | Relational fit for contacts/companies/tags/tasks |
...
### ActivityEntry (notes & activity timeline)
| Field | Type | Notes |
|---|---|---|
| id | UUID | PK |
| contact_id | UUID | FK → Contact |
...
3. **Contact Detail** — contact fields (editable), associated company, tags, custom fields, activity timeline (add note/call/email/meeting entry), open tasks for this contact (add/complete).
4. **Companies List** — searchable list of companies with contact counts; click → Company Detail.
5. **Company Detail** — company fields, list of associated contacts.
6. **Tasks** — cross-contact list of the current user's (and, for admins, all) open/completed tasks, filterable by due date/status.
7. **Contact/Company Create & Edit forms** — modal or dedicated page.
8. **CSV Import** — upload, column-mapping step, preview, confirm.
9. **Admin: User Management** (admin-only) — list users, invite/create user, change role, deactivate user.
10. **Admin: Tags & Custom Fields** (admin-only) — manage tag list and custom field definitions.
11. **My Account** — change own name/password.

## 5. Roles & Permissions

- **User**: full CRUD on contacts, companies, tags (assign existing), activity entries, tasks. Cannot manage other users or custom field/tag definitions.
...
- Add sales pipeline (deals/opportunities) as a v2 module?
